...
export function Template1Cover({ slide, settings }: TemplateProps) {
  return (
...
export function Template1Content({ slide, totalSlides, settings }: TemplateProps) {
  const primaryColor = settings.primary_color;
  const isFirstPage = !slide.content && !slide.bullets;

  if (isFirstPage || (!slide.content && !slide.bullets)) {
    return (
      <div className="w-full aspect-square relative overflow-hidden bg-white">
        <div className="absolute inset-0 flex items-center justify-center p-16">
          <h2 className="text-6xl font-bold text-gray-900 leading-tight text-center">
            {slide.title}
          </h2>
        </div>
...
export function Template1CTA({ slide, settings }: TemplateProps) {
  return (
    <div className="w-full aspect-square relative overflow-hidden bg-white">
      <div className="absolute inset-0 flex flex-col items-center justify-center p-16">
...
export function Template1End({ settings }: { settings: UserSettings }) {
  return (
    <div className="w-full aspect-square relative overflow-hidden bg-white">
      <div
        className="absolute inset-0"
